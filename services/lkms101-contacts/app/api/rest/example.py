"""
================================================================
Contact Service (MDM) - Example REST API
================================================================
File: services/lkms101-contacts/app/api/rest/example.py
Version: v2.0.0
Created: 2025-11-08
Updated: 2025-12-16
Description:
...
from app.models.enums import ContactType, ContactStatus, ContactPriority
...
async def list_contacts(
    ...
    """
    Get list of Contact entities with optional filtering.
...
async def get_contact(
    ...
    query = db.query(Contact).filter(Contact.id == item_id)
    ...
    if not item:
        raise HTTPException(
            ...
            detail=f"Contact with id {item_id} not found",
        )
    return item


@router.get("/code/{entity_code}", response_model=ContactResponse)
async def get_by_code(
    entity_code: str,
    include_deleted: bool = Query(False, description="Allow fetching soft-deleted records"),
    db: Session = Depends(get_db),
):
    """Get single Contact by human-readable code (e.g., TYA-2512-0001)."""
    query = db.query(Contact).filter(Contact.entity_code == entity_code)
    ...
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contact with code {entity_code} not found",
        )
    return item
...
async def create_contact(
    ...
    entity_code = generate_entity_code(db, item_data.type, Contact)

    # Create item with generated code
    item_dict = item_data.model_dump()
    item_dict["entity_code"] = entity_code
    item_dict["status"] = ContactStatus.OPEN  # Default status

    item = Contact(**item_dict)
    db.add(item)
    db.commit()
    db.refresh(item)

    # Publish Kafka event
    await publish_event("contact.created", {
        "id": item.id,
    ...
    logger.info(f"Created Contact id={item.id} code={item.entity_code}")
    return item
...
async def update_contact(
    ...
    # Publish Kafka event
    await publish_event("contact.updated", {
        "id": item.id,
        "entity_code": item.entity_code,
        "name": item.name,
    })

    logger.info(f"Updated Contact id={item.id}")
    return item
...
async def change_status(
    ...
    if new_status == ContactStatus.RESOLVED and not item.resolved_at:
        item.resolved_at = now
    elif new_status == ContactStatus.CLOSED and not item.closed_at:
        item.closed_at = now
    ...
    # Publish Kafka event
    await publish_event("contact.status_changed", {
        "id": item.id,
        "entity_code": item.entity_code,
    ...
    logger.info(f"Status changed Contact id={item.id}: {old_status.value} -> {new_status.value}")
...
async def soft_delete_contact(
    ...
    # Publish Kafka event
    await publish_event("contact.deleted", {
        "id": item.id,
        "entity_code": item.entity_code,
        "deleted_at": now.isoformat(),
    })

    logger.info(f"Soft deleted Contact id={item.id}")
...
async def restore_contact(
    ...
    # Publish Kafka event
    await publish_event("contact.restored", {
        "id": item.id,
        "entity_code": item.entity_code,
    })

    logger.info(f"Restored Contact id={item.id}")
...
async def hard_delete_contact(
    ...
    # Publish Kafka event
    await publish_event("contact.hard_deleted", {
        "id": item_id,
        "entity_code": entity_code,
    })

    logger.warning(f"HARD DELETED Contact id={item_id} code={entity_code}")
    return None
...
async def acquire_lock(
    ...
    logger.info(f"Lock acquired on Contact id={item_id} by user={user_id} ({user_name})")
...
async def release_lock(
    ...
    logger.info(f"Lock released on Contact id={item_id} (was held by: {previous_holder})")
