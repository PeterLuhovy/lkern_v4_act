 ...
 */
export type EntityFieldType = 'text' | 'textarea' | 'select' | 'number' | 'email' | 'datetime-local' | 'date' | 'url' | 'hidden';
/**
...
export interface EntityEditConfig<TEntity extends Record<string, unknown> = Record<string, unknown>> {
     ...
     */
    extractChanges?: (currentData: Record<string, unknown>, originalData: TEntity, sectionId: string) => Partial<TEntity>;
    /**
