...
export function TemplatePageDatagrid() {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: (item: TemplateItem) => {
        if (confirm(t('pages.template.deleteConfirm', { name: item.name }))) {
          alert(t('pages.template.deleteSuccess', { name: item.name }));
  ...
  const renderExpandedContent = (item: TemplateItem) => (
    <div className={styles.expandedContent}>
      ...
      <div className={styles.detailsGrid}>
        ...
        <div>
          <strong>{t('pages.template.details.date')}:</strong> {item.date}
        </div>
        <div>
          <strong>{t('pages.template.details.active')}:</strong> {item.isActive ? t('common.yes') : t('common.no')}
        </div>
      </div>
    </div>
  );
  ...
  const handleBulkExport = () => {
    ...
    alert(t('pages.template.bulkExport', { count: selectedRows.size }));
